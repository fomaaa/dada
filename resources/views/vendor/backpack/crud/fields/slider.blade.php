                ...
                @foreach( $field['columns'] as $prop => $label)
                    ...
                    <td>
                        @if($prop=='img')
                            ...
                            <input class="form-control input-sm" type="file" ng-model="item.{{ $prop }}">
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            $(document).ready(function () {
                ...
                $('input[type=file].form-control.input-sm').on('change', function () {
                    function getBase64(file, that) {
                        ...
                        reader.onload = function () {
                            ...
                            fieldvalue[0]['img'] = reader.result;
                            image = reader.result;
//                                console.log(JSON.stringify(lolvalue));
                            //var strinss = JSON.stringify(lolvalue);
                            //lol.val(strinss);
                            imgArr.push(reader.result);
                            console.log('imgarr',imgArr);
//                            that.setAttribute('type', 'text');
//                            that.setAttribute('type', 'file');
                        };
                    ...
                    var file64 = getBase64(file, this);

                });
                $('form').on('submit',function(ev){
                    if(imgArr.length) // Если картинки загружены в слайдер
                    {
                        ev.preventDefault();
                        var gallery = JSON.parse($('#gallery').val());
                        gallery.forEach(function(item,index){
                            gallery[index]['img'] = imgArr[index];
...
//                                console.log(JSON.stringify(fieldvalue));
                                //var strinss = JSON.stringify(fieldvalue);
                                //lol.val(strinss);
                                imgArr.push(reader.result);
                                console.log('imgarr',imgArr);
//                            that.setAttribute('type', 'text');
